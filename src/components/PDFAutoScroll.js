...
const PDFAutoScroll = ({ pdfViewerRef }) => {
  ...
  const performScroll = useCallback(() => {
    ...
    try {
      ...
      const zoomElement = document.querySelector('.zoom-controls span');
      ...
      if (isZoomedIn && !isAtBottom && currentPage < totalPages) {
        // Simulate pressing the 'p' key to scroll to the bottom of the page
        pdfContainer.scrollTo({
          top: pdfContainer.scrollHeight,
          behavior: 'smooth'
        });
        
        // Always play sound if enabled (similar to page changes)
        if (soundEnabled) {
          playSubtleBeep();
        }
  ...
  useEffect(() => {
    ...
    const handleKeyDown = (e) => {
      ...
      if (e.key === ',') {
        ...
        let scrollPos = 0;
        if (pdfContainer) {
          scrollPos = pdfContainer.scrollTop;
        }
      ...
      // Simulate page up with 'z' key (maintains scroll position like ',' key)
      if (e.key === 'z' || e.key === 'Z') {
        resetScrollTimer();
        
        // Get the previous page button but don't change scroll position
